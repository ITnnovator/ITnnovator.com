...
import { Plus, Trash2, X, Loader2, Image as ImageIcon, Briefcase, ChevronDown, ChevronUp } from 'lucide-react';
...
// ... (keep default export Component structure similar but update fields)
          ...
          <div key={item._id} className="relative group bg-[#111116] border border-white/5 p-6 rounded-2xl hover:border-white/20 transition-all flex flex-col">
            <div className="flex items-start justify-between mb-4">
              <div className="p-3 bg-blue-500/10 rounded-xl border border-blue-500/20">
                <img src={item.icon} alt={item.title} className="w-8 h-8 object-contain" onError={(e) => { e.target.onerror = null; e.target.src = '/webImages/icons.svg' }} />
              </div>
              <div className="flex gap-2">
                 <button onClick={() => handleEdit(item)} className="p-2 text-gray-500 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg transition-colors">Edit</button>
